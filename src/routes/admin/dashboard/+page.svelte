<script>
    ...
    function handleDelete(slug) {
        confirmModal = {
            ...
            onConfirm: async () => {
                try { await smartDeleteGift(slug);
                await loadData(); showToast("Data diarsipkan"); } 
    ...
    function changePlan(slug, e) {
        ...
        confirmModal = {
            ...
            onConfirm: async () => {
                await updateGiftPlan(slug, newPlan);
                await loadData(); showToast("Paket diubah");
            }
        };
    }
    ...
    async function downloadAssets(gift) {
        if (!gift) return;
        
        showToast("Memulai download aset...", "info");

        const assets = [];
        
        // Kumpulkan URL gambar kartu
        ['card1Image', 'card2Image', 'card3Image'].forEach(key => {
            if (gift[key]) assets.push({ url: gift[key], name: `${gift.slug}_${key}.jpg` });
        });

        // Kumpulkan URL gambar memories
        if (gift.memories && gift.memories.length > 0) {
            gift.memories.forEach((mem, index) => {
                if (mem.url) assets.push({ url: mem.url, name: `${gift.slug}_memory_${index + 1}.jpg` });
            });
        }

        if (assets.length === 0) {
            showToast("Tidak ada aset gambar untuk diunduh.", "warning");
            return;
        }

        // Download setiap aset
        for (const asset of assets) {
            try {
                // Fetch blob agar bisa didownload langsung tanpa membuka tab baru (jika CORS mengizinkan)
                ...
                // Beri jeda sedikit agar browser tidak memblokir download beruntun
                await new Promise(resolve => setTimeout(resolve, 500));
            } catch (error) {
                console.error("Gagal mendownload aset:", asset.url, error);
                // Fallback: Buka di tab baru jika fetch gagal (misal karena CORS)
                window.open(asset.url, '_blank');
            }
        }
        
        showToast("Proses download selesai.");
    }
</script>
    ...
    <div transition:fade class="fixed inset-0 z-[9999] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div transition:scale class="bg-white rounded-xl shadow-xl w-full max-w-xs p-5 text-center">
            ...
            <div class="flex gap-2">
                ...
                <button on:click={() => { confirmModal.onConfirm();
                confirmModal.show = false; }} class="flex-1 py-2 text-white rounded-lg text-xs font-bold {confirmModal.type==='danger'?'bg-rose-500':'bg-blue-500'}">Ya</button>
            </div>
        </div>
    </div>
{/if}
    ...
    <div transition:fade class="fixed inset-0 z-[9999] bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4">
        <div transition:scale={{start: 0.9}} class="bg-white w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col">
            <div class="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                ...
                <button on:click={() => showDetailModal = false} 
                class="w-8 h-8 rounded-full bg-slate-200 hover:bg-slate-300 flex items-center justify-center text-slate-600 transition">✕</button>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <div class="space-y-2 text-sm">
                            ...
                            <p><span 
                            class="text-slate-500">Untuk:</span> <b class="text-slate-800">{selectedGift.name}</b></p>
                            <p><span class="text-slate-500">Spotify:</span> <b class="text-green-600">{selectedGift.spotifyTitle ||
                            '-'}</b></p>
                            <p class="text-xs text-slate-400 truncate">{selectedGift.spotifyId ||
                            'No ID'}</p>
                        </div>
                    </div>
                    <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                        <h3 class="text-xs font-bold text-slate-400 uppercase 
                        mb-3">Isi Pesan Pendek</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {#each ['card1', 'card2', 'card3'] as card, i}
                            ...
                            <div class="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-2">
                                ...
                                {:else}
                                    <div class="w-full h-32 bg-slate-100 rounded-lg flex 
                                    items-center justify-center text-xs text-slate-400">No Image</div>
                                {/if}
                                <p class="text-xs text-slate-600 bg-slate-50 p-2 rounded h-20 overflow-y-auto border border-slate-100">{selectedGift[card+'Text'] ||
                                '-'}</p>
                            </div>
                        {/each}
                    </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            {#if selectedGift.memories && selectedGift.memories.length > 0}
                                {#each selectedGift.memories as mem, i}
                                    <div class="aspect-square bg-slate-100 rounded-lg overflow-hidden border 
                                    relative group">
                                        {#if mem.url}
                                            <img src={mem.url} alt="mem" class="w-full h-full object-cover transition transform group-hover:scale-105">
      
                                            <div class="absolute bottom-0 left-0 right-0 bg-black/60 p-1 text-[8px] text-white text-center truncate">{mem.label ||
                                            `Foto ${i+1}`}</div>
                                        {:else}
            ...
            <div class="p-4 border-t border-slate-100 bg-white flex justify-end gap-3">
                ...
                <button on:click={() => downloadAssets(selectedGift)} class="px-9 py-2 bg-pink-600 text-white rounded-lg font-bold text-sm hover:bg-pink-700 transition flex items-center gap-2">
...
<div class="h-screen w-full bg-slate-50 font-sans text-slate-800 flex overflow-hidden">
    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 shadow-2xl md:shadow-none transition-transform duration-300 ease-in-out transform {isSidebarOpen ?
    'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 md:w-64 md:flex flex-col">
        ...
        <nav class="flex-1 p-4 space-y-2">
            <button on:click={() => { activeTab = 'all'; isSidebarOpen = false; }} class="w-full flex items-center gap-3 px-4 py-3 font-bold rounded-xl border shadow-sm transition {activeTab === 'all' ?
            'bg-slate-50 text-pink-600 border-pink-100' : 'bg-white text-slate-500 border-transparent hover:bg-slate-50'}">
                <i class="fa-solid fa-chart-line"></i> Dashboard
            </button>
            <button on:click={() => { activeTab = 'pending'; isSidebarOpen = false; }} class="w-full flex items-center gap-3 px-4 py-3 font-bold rounded-xl border shadow-sm transition {activeTab === 'pending' ?
            'bg-orange-50 text-orange-600 border-orange-100' : 'bg-white text-slate-500 border-transparent hover:bg-slate-50'}">
                <i class="fa-solid fa-hourglass-half"></i> Pending
            </button>
            <button on:click={() => { activeTab = 'history'; isSidebarOpen = false; }} class="w-full flex items-center gap-3 px-4 py-3 font-bold rounded-xl border shadow-sm transition {activeTab === 'history' ?
            'bg-slate-100 text-slate-700 border-slate-200' : 'bg-white text-slate-500 border-transparent hover:bg-slate-50'}">
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden relative">
        <header class="h-20 absolute top-0 left-0 right-0 z-30 px-4 md:px-8 flex items-center justify-between bg-white/80 backdrop-blur-md border-b border-slate-200/60 sticky-top">
            <div class="flex items-center gap-4 flex-1 md:flex-none">
                <button on:click={() => isSidebarOpen = 
                true} class="md:hidden w-10 h-10 rounded-xl bg-white border border-slate-200 text-slate-600 flex items-center justify-center hover:bg-slate-50 transition shadow-sm"><i class="fa-solid fa-bars"></i></button>
        ...
        <div class="flex-1 
        overflow-y-auto p-4 md:p-8 pt-24 md:pt-28 pb-20 scroll-smooth">
            <div class="max-w-6xl mx-auto space-y-6">

                {#if activeTab === 'all'}
                    <div transition:slide class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <button on:click={() => showRevenueList = true} class="text-left w-full bg-white p-4 rounded-2xl 
                        border border-slate-100 shadow-sm hover:shadow-lg transition flex items-center gap-3 group">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden min-h-[400px]">
                    <div class="p-4 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
                        ...
                        <button on:click={loadData} disabled={isLoading} class="flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-500 
                        hover:text-pink-600 transition shadow-sm active:scale-95 disabled:opacity-50">
                            <i class="fa-solid fa-arrows-rotate {isLoading ?
                            'animate-spin' : ''}"></i>
                        </button>
                    </div>
                            ...
                            {:else}
                                ...
                                {#each pendingGifts as gift}
                                    <div class="flex items-center justify-between p-3 bg-white border border-orange-100 rounded-xl shadow-sm hover:border-orange-300 transition">
                                        ...
                                        <div class="flex gap-2">
                                            <button on:click={() => openDetail(gift)} class="text-[10px] px-3 py-1 bg-slate-100 rounded-lg hover:bg-slate-200 
                                            font-bold transition">Cek</button>
                                            ...
                                            <tr class="hover:bg-slate-50/80 transition group">
                                                ...
                                                <td class="px-6 py-4">
                                                    ...
                                                    <div class="relative w-32">
                                                        <select on:change={(e) => changePlan(gift.slug, e)} class="w-full appearance-none pl-3 pr-8 py-1.5 rounded-lg text-xs font-bold border cursor-pointer outline-none transition {gift.paymentPlan === 'lifetime' ?
                                                        'bg-purple-50 text-purple-700 border-purple-200 hover:border-purple-300' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-300'}">
                                                ...
                                                <td class="px-6 py-4">
                                                    <button on:click={() => toggleStatus(gift.slug, gift.isActive)} class="relative overflow-hidden text-[10px] font-bold px-3 py-1 rounded-full border transition group/status {gift.isActive ?
                                                    'bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-100' : 'bg-slate-100 text-slate-400 border-slate-200 hover:bg-slate-200'}">
                                                        <span class="relative z-10 flex items-center gap-1.5"><span class="w-1.5 h-1.5 rounded-full {gift.isActive ?
                                                        'bg-emerald-500' : 'bg-slate-400'}"></span>{gift.isActive ? 'ACTIVE' : 'OFF'}</span>
                                                    </button>
    ...
    <div transition:fade class="fixed inset-0 z-[9999] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div transition:scale class="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            <div class="p-4 border-b flex justify-between items-center bg-emerald-50">
                ...
                flex items-center gap-2"><i class="fa-solid fa-money-bill-wave"></i> Log Pemasukan</h3>
                ...
                {#each revenueGifts as gift}
                    ...
                    <div class="flex justify-between items-center p-3 border border-emerald-100 rounded-xl bg-white hover:bg-emerald-50 transition">
                        <div>
                            ...
                            <p class="text-[10px] text-slate-400 capitalize">{gift.paymentPlan} {gift.removeWatermark 
                            ? '+WM' : ''}</p>
                        </div>
