<script>
    ...
    // Ambil slug dari URL untuk navigasi balik
        ...
        try {
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    ...
                    text: 'Here is a photostrip memory for you! 脂'
                });
            } else {
    ...
    function reset() { step = 'upload';
        uploadedImages = []; resultDataUrl = null; }
    
    // Ganti fungsi goBack agar kembali ke halaman memories user
    function goBack() { 
        goto(`/${slug}/memories`);
    }
</script>
...
<div class="page-container">
    ...
    <div class="header">
        ...
        <h1 class="app-title">Create Photo - Special Paper</h1>
        ...
        <div class="content-area">
            ...
            {#if step === 'upload'}
                <div class="upload-section" in:fade>
                    ...
                    <div class="layout-selector">
                        <p class="label-text">Select Size :</p>
                    ...
                    <div class="actions">
                        <button class="btn-main" on:click={triggerUpload} disabled={uploadedImages.length >= MAX_PHOTOS}>
                            <i class="fa-solid fa-images"></i> Select Photo
                        </button>
                        {#if uploadedImages.length > 0}
                            <button class="btn-process" on:click={processPhotos} transition:scale>Make Memories</button>
                        {/if}
                    </div>
                </div>
            ...
            {:else if step === 'result'}
                <div class="result-section" in:fly={{ y: 50, duration: 800 }}>
                    <h3 class="success-title">Yeay! Ready to Post 🎉</h3>
...
<style>
    ...
    .nav-btn { width: 45px; height: 45px; background: rgba(255,255,255,0.1); border-radius: 50%; border: none; color: white; font-size: 1.2rem;
    ...
    .photo-slot { aspect-ratio: 1/1; background: #1f293782; border-radius: 16px; position: relative; overflow: hidden; display: flex; align-items: center;
    justify-content: center; }
    .empty-placeholder { width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; font-size: 2rem;
    color: #ffffff; cursor: pointer; }
    .thumb { width: 100%; height: 100%; object-fit: cover;
    }
    .remove-btn { position: absolute; top: 4px; right: 4px; width: 24px; height: 24px; background: #ef4444; color: white;
    border: none; border-radius: 50%; font-weight: bold; cursor: pointer; display: flex; align-items: center; justify-content: center; }

    .layout-selector { width: 100%; background: #1f293782; padding: 20px;
    border-radius: 20px; border: 1px solid #374151; }
    .label-text { font-size: 1rem; color: #9ca3af; margin-bottom: 12px; text-align: center;
    font-weight: bold; }
    .toggle-container { display: flex; gap: 10px; }
    .chip-btn { flex: 1;
    padding: 12px; font-size: 0.85rem; border-radius: 12px; border: 1px solid #4b5563; background: transparent; color: #d1d5db; cursor: pointer; transition: 0.2s; white-space: nowrap;
    ...
    .btn-main { background: #1f293782; color: white; padding: 18px; border-radius: 50px; font-weight: bold; border: none; cursor: pointer;
    display: flex; align-items: center; justify-content: center; gap: 10px; font-size: 1rem; }
    .btn-process { background: #ec4899; color: white;
    padding: 18px; border-radius: 50px; font-weight: bold; border: none; cursor: pointer; font-size: 1.1rem; box-shadow: 0 4px 15px rgba(236, 72, 153, 0.4);
    ...
    .spinner { width: 60px; height: 60px; border: 4px solid #374151; border-top-color: #ec4899; border-radius: 50%;
    ...
    .final-image { width: 100%; display: block;
    height: auto; }
    ...
    .btn-icon { background: #3b82f6; color: white; width: 55px; height: 55px; border-radius: 50%; font-size: 1.3rem; cursor: pointer;
    display: flex; align-items: center; justify-content: center; transition: 0.2s; }
    .btn-icon:active { background: rgba(255,255,255,0.1); transform: scale(0.9);
    }

    .btn-action { padding: 0 25px; height: 55px; border-radius: 50px; font-weight: bold; border: none; cursor: pointer;
    display: flex; gap: 8px; align-items: center; justify-content: center; flex: 1; max-width: 140px; font-size: 1rem; transition: transform 0.1s;
    }
    .btn-action:active { transform: scale(0.95); }

    .download { background: #db2777; color: white;
    box-shadow: 0 5px 15px rgba(196, 38, 165, 0.179); }
    .share { background: #3b82f6; color: white;
